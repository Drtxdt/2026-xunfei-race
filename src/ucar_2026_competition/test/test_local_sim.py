...
class LocalSimHelperTest(unittest.TestCase):
    ...
    def test_isolated_environment_replaces_main_master_and_ros_network_identity(self):
        ...
        self.assertEqual(environment["ROS_MASTER_URI"], "http://127.0.0.1:11312")
    ...
    def test_launch_command_is_local_and_waits_for_external_trigger(self):
        command = build_launch_command(True, "127.0.0.1", 26003)
        self.assertIn("target:=wait", command)
        self.assertIn("start_bridge:=true", command)
        self.assertIn("gui:=true", command)
        self.assertIn("bridge_host:=127.0.0.1", command)
        with self.assertRaises(LocalSimConfigError):
            build_launch_command(True, "0.0.0.0", 26003)
...
class LocalSimLaunchContractTest(unittest.TestCase):
    def test_reusable_launch_has_required_supervisor(self):
        ...
        self.assertEqual(args["sim_master_port"], "11312")
    ...
    def test_full_launch_defaults_to_local_sim_but_can_disable_it(self):
        ...
        expected = {
            ...
            "sim_master_port": "11312",
            "sim_bridge_host": "127.0.0.1",
