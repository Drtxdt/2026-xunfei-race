...
namespace ucarController
{
  ...
  public:
    MahonyFilter()
        : twoKp(2.0f), twoKi(0.0f), q0(1), q1(0), q2(0), q3(0),
          integralFBx(0), integralFBy(0), integralFBz(0) {}

    // 通过IMU数据更新四元数（融合加速度计与陀螺仪数据�????
    void updateIMU(float gx, float gy, float gz, float ax, float ay, float az, float dt = 0.01f)
    {
      // 计算加速度计的归一�????
    ...
    // 获取欧拉�????
    float getRoll() { return atan2(2.0f * (q0 * q1 + q2 * q3), 1.0f - 2.0f * (q1 * q1 + q2 * q2)); }
    float getPitch() { return asin(2.0f * (q0 * q2 - q3 * q1)); }
    float getYaw() { return atan2(2.0f * (q0 * q3 + q1 * q2), 1.0f - 2.0f * (q2 * q2 + q3 * q3)); }

  private:
    float twoKp;                                 // 比例增益
    float twoKi;                                 // �????分�?�益
    float q0, q1, q2, q3;                        // 四元�????
    float integralFBx, integralFBy, integralFBz; // �????分�??�????
  };
  ...
  {
    if (current_battery_percent_ == -1) // 初�?�值为 -1. 即没有获取到电量
    {
    ...
    {
      res.battery_state.power_supply_status = 2;     // 放电�???? 即�?�在运�??
      res.battery_state.power_supply_health = 1;     // �????�????
      res.battery_state.power_supply_technology = 0; // UNKNOWN
      res.battery_state.present = true;              // 存在电池
      boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_);
      res.battery_state.percentage = current_battery_percent_; // 电池电量百分�????
      lock.unlock();
      return true;
    }
  }
  ...
  float baseBringup::invSqrt(float number)
  {
    return 1.0 / sqrt(number);
  }

  void baseBringup::MahonyAHRSupdateIMU(float q[4], float gx, float gy, float gz, float ax, float ay, float az, float delta_s)
  {
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
    {
      ...
      if (twoKi > 0.0f)
      {
        ...
        // ���ֹ���
        integralFBx += twoKi * halfex * (delta_s);
        integralFBy += twoKi * halfey * (delta_s);
        integralFBz += twoKi * halfez * (delta_s);
    ...
    // ΢�ַ��̵������
    gx *= (0.5f * (delta_s)); // pre-multiply common factors
    gy *= (0.5f * (delta_s));
    gz *= (0.5f * (delta_s));
  ...
  void baseBringup::processIMU(uint8_t head_type)
  {
    ...
    else if (head_type == TYPE_GROUND || head_type == 0x50) // 无效数据，防�???�????�录失败
    {
      uint8_t ground_sn[1];
    ...
    if (head_type == TYPE_AHRS)
    {
      ...
      double magx, magy, magz, roll, pitch, yaw;
      // 以下都是磁力计数�????
      ...
      // roll = EulerAngle[2];
      // pitch = EulerAngle[1];
      // yaw = EulerAngle[0]; // 直接获取 Yaw �????

      // 使用tf库代替Eigen库可以获得yaw
      tf::Quaternion q(imu_data.orientation.w,
                       imu_data.orientation.x,
                       imu_data.orientation.y,
                       imu_data.orientation.z);
      tf::Matrix3x3 m(q);
      m.getRPY(roll, pitch, yaw); // ��ȡ Roll Pitch Yaw
      ...
      // pitch = mahonyFilter.getPitch();
      // yaw   = mahonyFilter.getYaw();
      // yaw = -yaw;
      // ROS_WARN("yawyawyaw:%f", yaw * 57.3);
      yaw = yaw + M_PI;
      // if (yaw < 0) {
      //   yaw = yaw + 2 * M_PI;
      // }else if (yaw > 2 * M_PI) {
  ...
  void baseBringup::quaternionToEuler(double Qw, double Qx, double Qy, double Qz,
                                      double &pitch, double &roll, double &yaw)
  {
    pitch = asin(2.0 * (Qw * Qx + Qy * Qz));
    roll = atan2(2.0 * (Qw * Qy - Qz * Qx), 1.0 - 2.0 * (Qx * Qx + Qy * Qy));
  ...
  void baseBringup::imuCallback(const sensor_msgs::Imu::ConstPtr &msg)
  {
    // 获取陀螺仪和加速度计的数据
    ...
    // 更新滤波�????
    mahonyFilter.updateIMU(gx, gy, gz, ax, ay, az, 0.01f);

    // 获取解算出的 Roll、Pitch、Yaw �????
    float roll = mahonyFilter.getRoll();
    float pitch = mahonyFilter.getPitch();
