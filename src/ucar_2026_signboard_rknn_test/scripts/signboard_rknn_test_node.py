#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RKNN/NPU YOLOv5 factory signboard inference with X11 debug image and speech."""
...
def resolve_model_path(param_path: str) -> str:
    ...
    try:
        ...
        default = os.path.join(yolo_dir, "models", "cls_best.rknn")
        if os.path.isfile(default):
            return default
    except Exception:
        pass
    rospy.logfatal("RKNN model not found. Set ~model_path or place cls_best.rknn under yolo/models")
    sys.exit(1)
...
class SignboardRknnTestNode:
    def __init__(self) -> None:
        repair_logging_levels()
        rospy.init_node("signboard_rknn_test_node")
        self.read_params()
    ...
    def read_params(self) -> None:
        ...
        self.detections_topic = rospy.get_param("~detections_topic", "/signboard_rknn_test/detections")
        self.debug_image_topic = rospy.get_param("~debug_image_topic", "/signboard_rknn_test/debug_image")
        self.status_topic = rospy.get_param("~status_topic", "/signboard_rknn_test/status")
