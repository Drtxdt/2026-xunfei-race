...
class TrackEndStopNode
{
public:
  TrackEndStopNode() : private_nh_("~")
  {
    loadParams();
    ...
    scan_sub_ = nh_.subscribe(radar_topic_, 1, &TrackEndStopNode::scanCallback, this);
    ...
    ROS_INFO("track_end_stop_node started: image=%s cmd_vel=%s debug=%s radar=%s threshold=%.3fm",
...
enum class State
  ...
  void loadParams()
  {
    ...
    private_nh_.param<std::string>("status_topic", status_topic_, "/track_end_stop/status");
    private_nh_.param<std::string>("debug_image_topic", debug_image_topic_, "/track_end_stop/debug_image");
    private_nh_.param<std::string>("debug_info_topic", debug_info_topic_, "/track_end_stop/debug_info");
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    {
      ROS_WARN_THROTTLE(2.0, "track_end_stop cv_bridge failed: %s", exc.what());
      return;
    }
    ...
    switch (state_)
    {
      ...
      case State::ObstacleShiftOut:
      {
        ...
        {
          setStatus("obstacle_shift_right");
      ...
      case State::ObstacleShiftBack:
      {
        ...
        if ((now - state_start_time_).toSec() < duration)
        {
          setStatus("obstacle_shift_left_return");
          cmd.linear.y = obstacle_lateral_speed_;
          publishCmd(cmd);
        }
        else
      ...
      case State::FinalStop:
        hardStop();
        setStatus((now - state_start_time_).toSec() >= end_stop_hold_ ? "finish" : "final_stop");
        if ((now - state_start_time_).toSec() >= end_stop_hold_)
          state_ = State::Finish;
        break;
  ...
  {
    FollowResult result = computeLeftmostLineFollow(mask);
  ...
  void publishFollowCommand(const FollowResult& follow, const ros::Time& now,
                            double speed_limit = -1.0, const char* status = "tracking")
  {
    if (!follow.found)
    {
                    ...
                    const EndOfTrackResult& end_result, const ros::Time& now)
  {
    ...
    {
      ROS_WARN_THROTTLE(2.0, "track_end_stop debug publish failed: %s", exc.what());
    }
  }
  ...
  void publishDebugInfo(const FollowResult& follow, const EndOfTrackResult& end_result, const ros::Time& now)
  {
    ...
    ROS_INFO_THROTTLE(0.5, "track_end_stop: %s", msg.data.c_str());
  }
...
{
  ros::init(argc, argv, "track_end_stop_node");
  TrackEndStopNode node;
  ros::spin();
  return 0;
}
