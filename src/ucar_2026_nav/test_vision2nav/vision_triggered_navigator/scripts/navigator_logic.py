...
def translation_corridor_width(samples, direction, min_forward=0.10,
                               ...
                               cluster_gap=0.08,
                               compact_max_span=0.30):
    """Return the narrowest gate formed by two compact obstacle clusters.

    Polar lidar points are projected into axes aligned with ``direction`` and
    clustered by spatial continuity.  A dangerous gate requires distinct,
    compact clusters on opposite sides at similar forward depth.  Continuous
    arena walls and a single wall crossing the motion centerline are therefore
    not mistaken for a pair of cones.

    The return value is ``(width, forward_distance, left_count, right_count,
    left_span, right_span)``.  When no compact two-sided gate is visible, all
    values are ``None``.
    """
    ...
    depth_tolerance = max(0.01, abs(float(depth_bin)))
    required = max(1, int(min_side_points))
    cluster_gap = max(0.01, abs(float(cluster_gap)))
    compact_max_span = max(cluster_gap, abs(float(compact_max_span)))
    points = []
    ...
    for angle, distance in samples or ():
        ...
        if (forward < min_forward or forward > max_forward or
            ...
            continue
        points.append((relative_angle, forward, lateral))
    ...
    for cluster in clusters:
        if len(cluster) < required:
            continue
        forwards = [item[1] for item in cluster]
        laterals = [item[2] for item in cluster]
        span = math.hypot(
            max(forwards) - min(forwards),
            max(laterals) - min(laterals),
        )
        if span > compact_max_span:
            continue
        # A cluster straddling the centerline is one obstacle, not a gate.
        if min(laterals) <= 0.0 <= max(laterals):
            continue
        compact.append({
            "side": "left" if min(laterals) > 0.0 else "right",
            "near_forward": min(forwards),
            "center_forward": 0.5 * (min(forwards) + max(forwards)),
            "edge": min(laterals) if min(laterals) > 0.0 else max(laterals),
            "count": len(cluster),
            "span": span,
        })

    narrowest = None
    left_clusters = [item for item in compact if item["side"] == "left"]
    right_clusters = [item for item in compact if item["side"] == "right"]
    for left in left_clusters:
        for right in right_clusters:
            if abs(left["center_forward"] -
                   right["center_forward"]) > depth_tolerance:
                continue
            ...
            candidate = (
                width,
                min(left["near_forward"], right["near_forward"]),
                left["count"],
                right["count"],
                left["span"],
                right["span"],
