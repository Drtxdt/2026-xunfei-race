...
namespace ucarController{
    //for read ===============
    #pragma pack(1)
    struct wheel_pluse_struct{ // 车轮脉冲数据（4个轮子）
        int16_t pluse_w1;
        int16_t pluse_w2;
        int16_t pluse_w3;
        int16_t pluse_w4;
    ...
    #pragma pack(1)
    struct read_pack_struct{ // 读帧格式（Read Frame）
        uint16_t head;                    //2 byte 帧头标识，用于帧同步
        uint8_t  ver;                     //1 byte 协议版本号
        uint8_t  len;                     //1 byte 帧数据长度
        struct   wheel_pluse_struct data; //8 byte 车轮脉冲数据(见上面)
        uint8_t  battery_percent;         //1 byte 电池百分比，单位为%，电池状态
        uint8_t  cs_code;                 //1 byte 校验码，用于校验数据完整性
    };                           //total 14 byte
    ...
    #pragma pack(1)
    struct write_pack_struct{ // 写帧格式（Write Frame）
        uint16_t head;                    //2 byte 帧头标识，用于帧同步
        uint8_t  ver;                     //1 byte 协议版本号
        uint8_t  len;                     //1 byte 帧数据长度
        struct   wheel_pluse_struct data; //8 byte 4轮脉冲 (int16 x4)
