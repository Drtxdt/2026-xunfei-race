...
用户：小飞小飞
小车：我在
用户：实体任务领取日用品，仿真环境需要食品
小车：好的
小车：开始导航
```

AIUI 只在“我在”播放完毕后开启，并在“好的”播放前关闭，避免小车把自己的播报识别成用户指令。旧 AIUI 云语义链路不再承担固定命令识别：语音节点将麦克风 PCM 发布到 `/speech_command_node/audio_pcm`，`fixed_command_iat.py` 使用讯飞流式听写取得整句文字。程序在讯飞返回最终句后，按“领取区/实体/现实/线下”和“仿真/模拟/虚拟/线上”等语义锚点分别抽取两个目标类别；实体和仿真的先后顺序、连接词及类别位于环境词前后均可变化。若两个目标类别相同，话术应明确包含“都要”“均为”“共同目标”或“相同”等词。只说一个目标不会被接受。一次空 VAD 结束后会自动建立下一轮听写，不会锁死；在收到有效双目标指令前再次说“小飞小飞”，小车会再次回复“我在”并重置听写会话。
